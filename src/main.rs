    ...
    for task in &tasks {
        for (start_tag, x0) in &task.starts {
            ...
            let runs: Vec<(&str, History)> = vec![
                (
                    "sgd_momentum",
                    sgd_momentum(task.f, x0, 1e-3, 0.9, 10_000, 1e-6),
                ),
                ("adam", adam(task.f, x0, 1e-2, 0.9, 0.999, 10_000, 1e-6)),
                ("bfgs", bfgs(task.f, x0, 1000, 1e-6)),
                (
                    "simulated_annealing",
                    simulated_annealing(task.f, &task.domain, 10_000),
                ),
            ...
            for (method, hist) in &runs {
                println!(
                    "{:20} iters={:6} f_final={:.6e}",
                    method,
                    hist.values.len().saturating_sub(1),
                    hist.last_value()
                );
            }
            ...
            let conv_title = format!(
                "Сходимость: {} (dim={}, start={})",
                task.name, task.dim, start_tag
            );
